import { Controller, Post, Body, BadRequestException, Req, Res, UnauthorizedException, Get, UseGuards } from '@nestjs/common';
...
export class UserController {
  ...
  @Post('register')
  async register(@Body() body: { name: string; email: string; password: string }, @Res() res: Response) {
    try {
      const { token, refreshToken, user } = await this.userService.register(body);
